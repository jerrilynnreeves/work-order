...
//Mails email content 
function mail_it($content, $subject, $recipient) 
{  
...
function submitWorkOrder($wo_storenumber, $wo_daterequested, $wo_category, $wo_equipment, $wo_problem, $wo_priority, $wo_requestedby, $wo_submittedby, $database)
{
	...
	if ($database)
	{
		...
		//Body: Problem | Requested By | Link to Workorder
			...
			mail_it($content, $subject, $recipient); 
...
function updateStatus($wonumber, $user, $status, $database)
{
	$run_query = 0;
	if ($database)
	{
		...
		if ($run_query == 1)
		{
			//Update status in the work order table
		...
		$run_dmemail = @mysqli_query ($database, $qry_dmemail);
			...
			while ($dm_row = mysqli_fetch_array($run_dmemail))
			{
				if ($dm_row[0]!=$assignedto_row[0])
				{
					echo "<p class='error'>Recp ".$recipient."</p>";
				}
			}
...
function getAssignNames ($wonumber, $database)
{	
...
{
	//set array for the role types that will be able to change the work work order assignment
	
	//query username against tbl_userassign for their name a role match
